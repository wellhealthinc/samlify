            ...
            xmlenc.encrypt(
              ...
              (err, res) => {
                ...
                const encryptAssertionNode = new dom().parseFromString(
                  `<${encAssertionPrefix}:EncryptedAssertion xmlns:${encAssertionPrefix}="${
                    namespace.names.assertion
                  }">${res}</${encAssertionPrefix}:EncryptedAssertion>`
                );
    ...
    decryptAssertion(here, entireXML: string) {
      return new Promise<string>((resolve, reject) => {
        ...
        if (encryptedAssertions.length !== 1) {
          throw new Error(
            `undefined number (${
              encryptedAssertions.length
            }) of encrypted assertions section`,
          );
        }
    ...
    async isValidXml(input: string) {
      return new Promise((resolve, reject) => {
        ...
        validator.validateXML(
          input,
          path.resolve('saml-schema-protocol-2.0.xsd'),
          (err, result) => {
            if (err) {
